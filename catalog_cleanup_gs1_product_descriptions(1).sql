...
WITH pcs AS (
  SELECT liam,
    ...
    CONCAT(IF(brand.brand_displayable=TRUE,CONCAT(brand.name_fr," "),''),
      ...
      IF(attributes.items_per_package > 1, CONCAT(CAST(attributes.items_per_package AS STRING),"x"),""),
      attributes.item_size, 
      attributes.item_size_uom) AS long_name_fr, -- String of text found on PLP
      description_en,
      description_fr,
      total_stores
  ...
  WHERE status = "APPROVED"
    ...
    AND (LENGTH(IFNULL(description_en,"")) < 100 OR LENGTH(IFNULL(description_fr,"")) < 100) -- Either english or french descriptoin is short
),
...
gs1 AS (
  SELECT DISTINCT gtin_cd AS UPC,
    ...
    IFNULL(CONCAT("<p>",REPLACE(REGEXP_REPLACE(brnd_own_prod_mkt_eng_msg,"\n","</p><p>"),"</p><p></p>","</p>"),"</p>"),"") AS clean_mm_en,
    ...
    IFNULL(CONCAT("<p>",REPLACE(REGEXP_REPLACE(brnd_own_prod_mkt_fr_msg,"\n","</p><p>"),"</p><p></p>","</p>"),"</p>"),"") AS clean_mm_fr,
...
SELECT pcs.*,
  gs1.*,
  CASE WHEN LENGTH(description_en) > 100 THEN description_en
    WHEN addedfeat_and_bnft_eng_desc = brnd_own_prod_mkt_eng_msg THEN clean_fab_en
    ELSE CONCAT(IFNULL(clean_mm_en,""), clean_fab_en) END AS new_desc_en,
  CASE WHEN LENGTH(description_fr) > 100 THEN description_fr 
    WHEN addedfeat_and_bnft_fr_desc = brnd_own_prod_mkt_fr_msg THEN clean_fab_fr
    ELSE CONCAT(IFNULL(clean_mm_fr,""), clean_fab_fr) END AS new_desc_fr
FROM pcs
  JOIN `lt-dia-lake-prd-raw.product.sap_mara_curr_v` ON article_number = LTRIM(matnr,'0')
  JOIN gs1 ON ean11 = UPC
WHERE  
   ((LENGTH(IFNULL(description_en,"")) < 100 AND LENGTH(CONCAT(IFNULL(clean_mm_en,""), clean_fab_en)) > 1)
  OR (LENGTH(IFNULL(description_fr,"")) < 100 AND LENGTH(CONCAT(IFNULL(clean_mm_fr,""), clean_fab_fr)) > 1)) -- Either EN or FR description was short and a GS1 alternative was found
  AND (IFNULL(description_en,"") <> CONCAT(IFNULL(clean_mm_en,""), clean_fab_en)
    OR IFNULL(description_fr,"") <> CONCAT(IFNULL(clean_mm_fr,""), clean_fab_fr)) -- GS1 alternative is different than current description

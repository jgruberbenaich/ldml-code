...
case  when Merchant ='Real Canadian Superstore' and Flyer_Run_Name ='ONT' then Concat(split(wff.Correct_Articles,'_')[safe_ordinal(1)],'RCSO')
      ...
      when Merchant ='Provigo' then Concat(split(wff.Correct_Articles,'_')[safe_ordinal(1)],'PROC')
 ...
 case  when Merchant ='Real Canadian Superstore' and Flyer_Run_Name ='ONT' then 'superstoreo'
  ...
  FROM `ld-pcx-bia.Merch_Flyer.WEEKLY_FLIPP_FLYER`  wff
...
ON ltrim(H.id,'0') = S.store_number
 ...
 AND DISMM in ('RE', 'ND', 'ZA')
  ...
  GROUP BY 1,2,3,4)
select bt.Banner, bt.Region,bt.pg,bt.correct_articles, forcebanner, rp.mch3_name,rp.DISMM from base_table as bt left join RP_Check rp on bt.RPcheckindex = rp.article_banner